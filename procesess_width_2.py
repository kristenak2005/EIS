...
def plot_velocity(linepair, velocity_map, aia_map, hmi_map, output_location):
    #date = comp.date.strftime("%Y%m%d_%H%M%S")
...
def plot_width(linepair, width_map, aia_map, hmi_map, output_location):
    #date = comp.date.strftime("%Y%m%d_%H%M%S")
    date = width_map.date.strftime("%Y%m%d_%H%M%S")
    if linepair == "fe_12_195":
        title = f"Width for {linepair} " + date
    else:
        title = f"Width for {linepair} " + date
    
    if width_map.dimensions[1]/width_map.dimensions[0] >= 20:
        figs = (12,5)
        wid_rat = [1,3,3]
        asp = 1/4
    else:
...
# Width
    ...
    width_map.plot_settings['norm'].vmin = -15
    width_map.plot_settings['norm'].vmax = 15
...
# %%
    ...
    line_list = ["fe_12_195"]#"s_10_264"]#,"fe_12_195","fe_13_202","fe_13_203"]
    ...
    for event in range(0, len(eis_evts)):
        ...
        for file in f_eis:
           ...
           # plot_composition('SiS', m_SiS, aia_map, hmi_map, output_location+'/plots')
            ...
            m_width = w_map
            plot_width(wvl, m_width, aia_map, hmi_map, output_location+'/plots')
